...
## Bash script to setup PX4 development environment on debian LTS (11, 10, 9).
...
if [[ ! -f "${DIR}/${REQUIREMENTS_FILE}" ]]; then
	echo "FAILED: ${REQUIREMENTS_FILE} needed in same directory as debian.sh (${DIR})."
	return 1
fi
...
if [[ "${DEBIAN_RELEASE}" == "8" ]]; then
	echo "Debian 8 is no longer supported"
	exit 1
elif [[ "${DEBIAN_RELEASE}" == "9" ]]; then
	echo "Debian 9 is no longer supported"
	exit 1
elif [[ "${DEBIAN_RELEASE}" == "10" ]]; then
	echo "Debian 10"
elif [[ "${DEBIAN_RELEASE}" == "11" ]]; then
	echo "Debian 11"
fi
...
if [[ "${DEBIAN_RELEASE}" == "9" ]]; then
	echo "Installing debian 9 PX4-compatible ccache version"
